  ...
  const userTypes = [
    {
      ...
      title: 'Citizen',
      description: 'Book waste pickup slots, earn eco-points, and redeem rewards',
      icon: Users,
      bgColor: 'bg-gradient-to-br from-citizen-green/10 to-eco-light/20',
      iconColor: 'text-citizen-green',
      features: ['Book Pickup Slots', 'Track Bookings', 'Earn Eco-Points', 'Redeem Rewards']
    },
    {
      ...
      features: ['Manage Pickup Slots', 'View Ward Bookings', 'Update Collection Status', 'Track Performance']
    },
    {
      ...
      features: ['User Management', 'Analytics Dashboard', 'Ward Management', 'Policy Oversight']
    }
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {userTypes.map((userType, index) => {
            ...
            return (
              <Card 
                key={userType.type}
                className={`user-type-card ${userType.bgColor} animate-slide-up`}
                style={{ animationDelay: `${index * 0.2}s` }}
                onClick={() => onUserTypeSelect(userType.type)}
              >
                <div className="space-y-6">
                  {/* Icon */}
                  <div className={`w-16 h-16 mx-auto rounded-full bg-white shadow-lg flex items-center justify-center ${userType.iconColor}`}>
                    <Icon size={32} />
                  </div>
                  ...
                  <div className="space-y-2">
                    {userType.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center gap-2 text-sm text-muted-foreground">
                        <div className={`w-2 h-2 rounded-full ${userType.iconColor.replace('text-', 'bg-')}`}></div>
                        <span>{feature}</span>
                      </div>
                    ))}
                  </div>
